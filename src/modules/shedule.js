...
module.exports = async function (type) {
    ...
    switch (type) {
        case 0:
            ...
            break;
        case 1:
            let nextday = nextDay().toLocaleDateString();
...
function getHomeworksForDay(r, date) {
    for (let i = 0; i < r.length; i++) {
        if (r[i].date == date) {
            if (r[i].homeworks.length > 0) {
                let out = "";
                for (let a = 0; a < r[i].homeworks.length; a++) {

                    out += a + ". " + r[i].homeworks[a].discipline + " -  " +
                        r[i].homeworks[a].homework + "\n"
                }
                return out
            } else {
                return "выходной"
